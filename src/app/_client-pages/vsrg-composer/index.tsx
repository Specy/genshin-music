...
const VsrgComposerCanvas = dynamic(() => import("$cmp/pages/VsrgComposer/VsrgComposerCanvas").then(m => m.VsrgComposerCanvas), {
    ssr: false,
    loading: () => <div style={{ width: '100%', height: '100%', background: 'var(--primary)'}}></div>
});
...
import {clamp, isFocusable} from "$lib/utils/Utilities";
...
type VsrgComposerProps = {
    ...
    registerLeaveHandler: AppNavigation['registerLeaveHandler']
...
class VsrgComposer extends Component<VsrgComposerProps, VsrgComposerState> {
    ...
    componentDidMount() {
        ...
        this.cleanup.push(this.props.registerLeaveHandler(this.prepareToLeave))
    ...
    prepareToLeave = async (): Promise<boolean> => {
        ...
        if (settings.autosave.value) return (await this.saveSong()) !== null

        const shouldSave = await asyncConfirm(this.props.t('question:unsaved_song_save', {song_name: vsrg.name}), true)
        if (shouldSave === null) return false
        if (!shouldSave) return true
        return (await this.saveSong()) !== null
    }
    changePage = async (page: string | 'Home') => {
        if (page === 'Home') return homeStore.open()
        await this.props.navigation.push(page)
    }
...
export default function VsrgcomposerPage() {
    ...
    return <VsrgComposer
        ...
        registerLeaveHandler={navigation.registerLeaveHandler}
        t={t}
    />
}
