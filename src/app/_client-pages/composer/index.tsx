...
const ComposerCanvas = dynamic(() => import("$cmp/pages/Composer/ComposerCanvas"), {
    ssr: false,
    loading: () => <div className={'canvas-wrapper'} style={{ backgroundColor: 'var(--primary)'}}></div>
})
...
import {calculateSongLength, delay, formatMs} from "$lib/utils/Utilities"
...
class Composer extends Component<ComposerProps, ComposerState> {
    ...
    prepareToLeave = async (): Promise<boolean> => {
        ...
        const shouldSave = await asyncConfirm(this.props.t('question:unsaved_song_save', {song_name: song.name}), true)
...
export default function ComposerPage({inPreview, songId}: ComposerPageProps) {
    ...
    return <Composer
        ...
        t={t}
        songId={querySongId ?? songId ?? null}
        showMidi={Boolean(showMidi)}
