...
<BaseBlogPost metadata={addToHomeScreenMetadata}>
  <div>
    <p class="blog-p">
      Adding the app to the home screen will make it behave more like an actual native app. It will
      go full screen and create an icon to quickly access the app. In certain browsers (like chrome
      or chromium based) it will also allow more features, like the ability to open the files
      created by the app.
    </p>
    {#if pwaStore.state.installEvent}
      <p class="blog-p">
        Your browser allows for the app to be quickly installed, just press this button and then
        press the "install" button that will pop up in your browser.
    ...
    {/if}
    <Header type="h2" margin="1rem 0">How to install the app in different devices</Header>
    ...
    <Column gap="0.8rem">
      ...
      <ExpandableContainer>
        ...
        {/snippet}
        <p class="blog-p">
          If you are on Mac, i suggest using the app with chrome or chromium based browsers instead
          of safari.
        </p>
        <BlogImage
