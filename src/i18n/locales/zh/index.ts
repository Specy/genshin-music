export const i18n_zh = {
    logs: {
        ...
        error_importing_file: `文件导入失败 "{{file_name}}}"`,
        ...
        converting_recorded_to_composed_warning: '将录制的歌曲转换为作曲，音频可能不准确',
    },
    question: {
        unsaved_song_save: `你修改了这个歌曲: "{{song_name}}" 是否保存? 不保存，你的修改将会丢失。`,
    ...
    home: {
        app_description: "{{APP_NAME}}是一个可以进行创作、演奏、播放的程序。",
        add_to_home_screen: "程序要使网络，并全屏显示，请将网站添加到桌面",
        clear_cache_warning: "清除浏览器缓存 / 可能会删除您保存的歌曲，请确保进行备份",
        persistent_storage_button: "单击下面的按钮，如果您内存不够时，确保浏览器不会删除您的歌曲",
        privacy_policy: "我们使用cookie进行分析，如果继续使用此应用程序，则表示您同意我们使用cookie，了解更多信息：",
        no_affiliation: "此应用不隶属于{{company_name}}，它是一个由粉丝制作的程序。",
        composer_description: "与成熟的音乐作曲家一起创作或编辑歌曲，MIDI吧！",
        player_description: "播放、下载、录制和导入歌曲。通过循环模式和练习模式来学习歌曲。",
        ...
        composer_name: "作曲",
        player_name: "演奏",
        vsrg_composer_name: "VSRG编写器",
        donate_name: "捐赠",
        vsrg_player_name: "VSRG播放器r",
