...
{#snippet faMusicIcon()}
    <svg
        ...
        width="1em"
        xmlns="http://www.w3.org/2000/svg"
    ><path d="M470.38 1.51L150.41 96A32 32 0 0 0 128 126.51v261.41A139 139 0 0 0 96 384c-53 0-96 28.66-96 64s43 64 96 64 96-28.66 96-64V214.32l256-75v184.61a138.4 138.4 0 0 0-32-3.93c-53 0-96 28.66-96 64s43 64 96 64 96-28.65 96-64V32a32 32 0 0 0-41.62-30.49z"/></svg>
{/snippet}
...
>
    {#if browserHistoryStore.hasNavigated && !open}
        <MenuButton ariaLabel={t('menu:go_back')} style="margin-bottom:auto" onclick={() => window.history.back()}>
            {@render faArrowLeftIcon()}
        </MenuButton>
    {/if}
    {#if open}
        <MenuButton ariaLabel={t('menu:close_menu')} style="margin-bottom:auto" onclick={() => open = false}>
            {@render faTimesIcon()}
        </MenuButton>
    {/if}
