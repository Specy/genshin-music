<script lang="ts">
    ...
    const instrumentLabels = $derived(game.instruments.list.map((i) => ({value: i, label: t(`instruments:${i}`)})))
...
{#snippet ioMdMusicalNoteIcon()}
    <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" height="18" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M256 64v225.1c-12.6-7.3-27.1-11.7-42.7-11.7-47.1 0-85.3 38.2-85.3 85.3s38.2 85.3 85.3 85.3 85.3-38.2 85.3-85.3V149.3H384V64H256z"/></svg>
{/snippet}

{#snippet mdPianoIcon()}
    <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" height="18" width="18" xmlns="http://www.w3.org/2000/svg"><path fill="none" d="M0 0h24v24H0z"/><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 11.5h.25V19h-4.5v-4.5H10c.55 0 1-.45 1-1V5h2v8.5c0 .55.45 1 1 1zM5 5h2v8.5c0 .55.45 1 1 1h.25V19H5V5zm14 14h-3.25v-4.5H16c.55 0 1-.45 1-1V5h2v14z"/></svg>
{/snippet}
...
>
    ...
    {/snippet}
    <MenuButton ariaLabel={t('menu:toggle_menu')} style="margin-bottom:auto" onclick={() => isVisible = !isVisible}>
        {@render faTimesIcon()}
    </MenuButton>
    ...
    {#snippet panel()}
        <MenuPanelWrapper>
            <MenuPanel title={t('menu:settings')} id="Settings">
                <SettingsPane settings={settings} onUpdate={handleSettingChange} changeVolume={onVolumeChange} />
            </MenuPanel>
        </MenuPanelWrapper>
    {/snippet}
</MenuSidebar>
