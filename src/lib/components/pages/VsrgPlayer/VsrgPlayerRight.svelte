<script lang="ts">
    ...
    // Old: src/components/pages/VsrgPlayer/VsrgPlayerRight.tsx (29 lines) +
    // VsrgPlayerRight.module.css (8 lines, single class, ported verbatim below).
    //
    // `if (!song) return null` gated the WHOLE component in old, INCLUDING the sibling
    // `<VsrgPlayerScore/>` (both live in the same `return` block) - preserved exactly: the
    // `{#if song}` below wraps both the stop/retry button row AND `<VsrgPlayerScore />`, so the
    // score overlay also disappears whenever no song is loaded, matching old.
    //
    // `MemoizedIcon`/`IconButton` (react-icons/fa's FaStop/FaSyncAlt, old: bare, no size/className)
    // -> two local snippets. FaStop byte-matched against the copy already inlined in
    // PlayerSongControls.svelte (P4b Task 5, same pinned react-icons@5.6.0 source); FaSyncAlt is
    // new to this migration (fetched fresh from the same pinned unpkg.com/react-icons@5.6.0/fa/
    // index.mjs source).
