...
export class VsrgCanvasCache {
  ...
  constructor({ app, colors, sizes, trackColors, isHorizontal, playbarOffset }: VsrgCacheProps) {
    this.textures = {
      hitObjects: {},
      heldHitObjects: {},
      selectionRings: {},
  ...
  generateOthers(app: Application) {
    ...
    square
      .rect(0, 0, this.sizes.width, this.sizes.timelineSize)
    ...
    note
      .circle(sizes.timelineSize / 2 + 1, sizes.timelineSize / 2, noteSize / 2)
    ...
    if (isHorizontal) {
      time
        .roundRect(margin / 2, margin / 2, sizes.width / 2 - margin, sizes.height / 2 - margin, 16)
      ...
      time
        .roundRect(margin / 2, margin / 2, sizes.width / 2 - margin, sizes.height / 3 - margin, 8)
        .fill({ color: colors.background_10[1] });
      const timeTexture = app.renderer.generateTexture({
        target: time,
        resolution: 1,
  ...
  generateTrails(app: Application) {
    ...
    const hitObjectHeight = clamp(
      (sizes.keyHeight / 2) * sizes.scaling,
      sizes.keyHeight / (this.isMobile ? 3 : 4),
      100
    );
    ...
    withError.forEach((color) => {
      ...
      if (this.isHorizontal) {
        trail
          .rect(0, margin / 2, hitObjectHeight, hitObjectHeight - margin)
          .fill({ color: Color(color).rgb().rgbNumber() });
      } else {
        trail
          .rect(margin / 2, 0, hitObjectHeight - margin, hitObjectHeight)
          .fill({ color: Color(color).rgb().rgbNumber() });
      }
  ...
  generateSelectionRings(app: Application) {
    ...
    const withError = [...trackColors, '#FF0000'];
    const hitObjectHeight = clamp(
      (sizes.keyHeight / 1.5) * sizes.scaling,
      sizes.keyHeight / (this.isMobile ? 1.2 : 2),
      150
    );
    withError.forEach((color) => {
      const ring = new Graphics();
      ring
        .circle(hitObjectHeight / 2, hitObjectHeight / 2, hitObjectHeight / 2 - 3)
  ...
  generateTrackCache(app: Application) {
    ...
    const hitObjectHeight = clamp(
      (sizes.keyHeight / 1.8) * sizes.scaling,
      sizes.keyHeight / (this.isMobile ? 2 : 3.5),
      100
    );
    const withError = [...trackColors, '#FF0000'];
    withError.forEach((color) => {
      ...
      hitObject
        .circle(circleSize, circleSize, circleSize - 5)
        .fill({ color: colors.background_10[1] })
      ...
      heldHitObject
        .roundRect(margin, margin, diamondSize, diamondSize, 6)
        .fill({ color: colors.background_10[1] })
        .stroke({ width: 5, color: Color(color).rgb().rgbNumber() });
  ...
  generateSnapPoints(app: Application) {
    ...
    if (isHorizontal) {
      small.moveTo(0, 0).lineTo(0, sizes.height).stroke({ width: 2, color: colors.lineColor[1] });
      ...
      large.moveTo(0, 0).lineTo(0, sizes.height).stroke({ width: 4, color: colors.secondary[1] });
