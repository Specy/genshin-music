...
<!-- QUIRK: this row is a disclosed outlier from its ComposerSongRow/PlayerSongRow siblings, kept
     byte-for-byte rather than normalized to match them: almost every string below is hardcoded
     English, never routed through t() ("Invalid song", the tooltips, the dropdown-row labels,
     "None", the delete confirmation); and several icon sizes below (FaPen/FaDownload/FaTrash at
     14px vs the siblings' 1em) also differ from the sibling files. Not oversights to "fix"
     piecemeal. -->
{#if data.type !== 'vsrg'}
  <div class="row">Invalid song</div>
{:else}
