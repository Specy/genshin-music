...
<Row align="center" justify="between" class="track-modifier" style="background-color:{color.hex()};{style}">
    <span class="text-ellipsis">
        {data.alias}
    </span>
    <Row align="center" justify="end">
        <AppButton class="flex-centered vsrg-track-modifier-button" onclick={() => onVisibilityChange(!data.hidden)}>
            {#if data.hidden}
        ...
        <AppButton class="flex-centered vsrg-track-modifier-button" style="margin-left:0.3rem" onclick={() => onChange(data.set({muted: !data.muted}))}>
            {#if data.muted}
