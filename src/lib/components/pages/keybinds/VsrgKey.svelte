...
<button
  ...
  use:clickOutside={{
    ...
    onOutside: () => handleClick(false),
  }}
  style="width:3.5rem;font-size:1rem;height:3.5rem;margin:0.4rem;border:none;background-color:{isActive
    ? 'var(--accent)'
    : 'var(--primary)'};color:{isActive
    ? 'var(--accent-text)'
    : 'var(--primary-text)'};cursor:pointer"
