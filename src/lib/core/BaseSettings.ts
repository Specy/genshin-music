...
// MIDIShortcutName relocated to $core/utils/Utilities in Task 5 (this file used to define it
// locally - deleted, see MIDISettings below) - re-imported/re-exported here alongside
// MIDIShortcut; reconcile like Utilities.ts's own note at its definition site.
import {MIDIShortcut} from "./utils/Utilities"
export type {MIDIShortcutName} from "./utils/Utilities"
import type {
    SettingsCheckbox,
    SettingsInstrument,
    SettingsNumber,
...
// VsrgSongKeys/VsrgKeyboardLayout hoisted into $core/types.ts by this task (see Step 3 note in
// task-6-brief.md) - VsrgSong.ts (Task 7) and VsrgPlayerKeyboard (UI phase) don't exist yet in
// this tree, so both types are sourced from the single $core/types.ts home instead.
import type {VsrgSongKeys, VsrgKeyboardLayout} from "$core/types"
...
export const PlayerSettings = {
    other: {
        settingVersion: APP_NAME + 90 //change when instrument is added
    },
    data: {
        instrument: {
...
export const ZenKeyboardSettings = {
    other: {
        settingVersion: APP_NAME + 34 //change when instrument is added
    },
    data: {
        instrument: {
            name: "zen_instrument",
