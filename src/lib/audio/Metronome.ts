...
class Metronome {
    ...
    async loadBuffers() {
        ...
        const promises = [
            fetchAudioBuffer(`${base}/assets/audio/MetronomeSFX/bar.mp3`, this.audioContext).catch(() => this.emptyBuffer),
            fetchAudioBuffer(`${base}/assets/audio/MetronomeSFX/quarter.mp3`, this.audioContext).catch(() => this.emptyBuffer)
        ]
