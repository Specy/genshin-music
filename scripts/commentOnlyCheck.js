...
function diffHashes(before, after) {
  ...
  const changed = [...beforePaths]
    .filter((p) => afterPaths.has(p) && before[p] !== after[p])
    .sort();
...
function runSnapshot() {
  ...
  const snapshot = {
    excluded: HASH_EXCLUSIONS.map((exclusion) => exclusion.description),
    files: hashes,
  };
...
function runCompare() {
  ...
  if (!fs.existsSync(SNAPSHOT_PATH)) {
    console.error(
      `No snapshot at ${SNAPSHOT_PATH} - run "node scripts/commentOnlyCheck.js snapshot" first.`
    );
    process.exit(1);
  }
