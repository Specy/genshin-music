...
function buildGenshin() {
  execSync('npm run build:genshin', {
    stdio: 'inherit',
    env: { ...process.env, ...FIXED_BUILD_ENV },
  });
}
...
function extractClassAttributes(html) {
  ...
  if (/\sclass\s*=\s*'/.test(scrubbed)) {
    throw new Error(
      'Found a single-quoted class attribute - extend extractClassAttributes() to handle it rather than silently skipping it.'
    );
  }
  if (/\sclass\s*=\s*[^"'\s>]/.test(scrubbed)) {
    throw new Error(
      'Found an unquoted class attribute - extend extractClassAttributes() to handle it rather than silently skipping it.'
    );
  }
...
function diffClassAttrs(before, after) {
  ...
  const changed = sortKeys(
    [...beforeKeys].filter((k) => afterKeys.has(k) && beforeFlat[k] !== afterFlat[k])
  );
...
function runSnapshot() {
  ...
  console.log(
    `Snapshot written: ${Object.keys(files).length} files, ${countAttrs(files)} class attributes -> ${SNAPSHOT_PATH}`
  );
  process.exit(0);
}
...
function runCompare() {
  if (!fs.existsSync(SNAPSHOT_PATH)) {
    console.error(
      `No snapshot at ${SNAPSHOT_PATH} - run "node scripts/classAttrCheck.js snapshot" first.`
    );
    process.exit(1);
  }
  ...
  if (!added.length && !removed.length && !changed.length) {
    console.log(
      `No differences from snapshot (${Object.keys(after).length} files, ${countAttrs(after)} class attributes compared).`
    );
    process.exit(0);
  }
  if (added.length) {
