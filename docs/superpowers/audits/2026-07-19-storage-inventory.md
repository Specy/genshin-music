...
| `` rg -n "APP_NAME \+                                            | `\$\{APP_NAME\}" src -g '_.ts' -g '_.tsx' `` | 42                | 10  |
...
Settings-blob `settingVersion` values (embedded _inside_ the JSON content of
...
prefix applied uniformly; it does not change route _structure_.
...
Only the _source_ of the two interpolated values changed, not the template or
