...
| Packaging               | Version bumps: **composed v4, recorded v3, VSRG v2**. All older versions deserialize forever (converted to the new model on load, quirks preserved). Downloads use the current format for every game. The dormant Sky old-format exporter is retained in code for a possible future explicit legacy-export option, but no download path calls it.                                                                                                                                                                                                                                                                |
...
- **Dormant Sky old-format export**: `toOldFormat()` reverse-maps id → default-15-key index and drops unmappable ids. It is retained and tested as restoration-ready compatibility code, but downloads always call `serialize()`.
