...
**IMPLEMENTED 2026-08-19/20, phases A–E** (both games green, parity fixtures untouched).
...
- **E — Deletions & docs**: `toOldFormat`+`legacyColumnsView`+`countOldFormatDroppedNotes` commented out; `MIDI_MAP_TO_NOTE`/`NOTE_MAP_TO_MIDI` → arithmetic; dead nominal-only helpers pruned; changelog (+ third-party `version` warning), i18n, memory/docs sync. SHIPPED 2026-08-19. Four findings: (1) `NOTE_MAP_TO_MIDI` was ALREADY dead before this phase — C6's honest export writes the stored number, so nothing had asked for a nominal→MIDI lookup since the flip; only the forward map still had consumers. (2) The replacement snap is derived from `CANONICAL_NOTE_IDS`, not from a white-key constant: a game's grid IS its scale, so an accidental is a pitch class the grid has no row for and it snaps to the nearest grid id at or below — one source of truth, no game-id branch, and byte-parity is PROVEN rather than assumed by rebuilding the frozen v1 config surface's copy of both tables out of the arithmetic. (3) `numberToNominal` went with the export it served (its only two callers were the two `nominalOf`s); the swaps/grid/legacy paths never needed the way back. (4) The changelog mirror is not English-only: the nine translated locales key changes by INDEX, so inserting lines before the catch-all required renaming their `change-8` to `change-12` — otherwise every translated app would print its bug-fix line where the format announcement belongs.
