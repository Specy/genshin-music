...
- The id is a *name in a shared namespace*, not a promise about sound: pitch-true MIDI export would require adding separate sounding-pitch data to instruments (deliberately out of scope).
- A note whose id the current instrument doesn't offer is a **stranded note**: skipped at playback and marked in the composer, never rewritten. Two exceptions, both explicit imports (discovered during implementation: the historic `IMPORT_NOTE_POSITIONS` remap was a *rank-preserving* map — a uniform -12 id shift for the default instruments, e.g. Sky 60 → Genshin 48 — not a fold of only out-of-range notes):
  ...
  - **New-format files** carry their ids; the roster swaps to the target's default instrument and only out-of-range ids octave-fold (Sky's 84 → 72), fold collisions dedupe, and ids landing on scale gaps stay stranded.
